...
// 服务器地址
window.serverUrl = "http://192.168.0.111:8888";
...
window.openWinAnimationType = "push";
...
window.showProgressText = "请稍等...";
window.ajaxTimeout = 3000;
// APP默认参数 END
// 用户偏好设置的Key
window.userKey = "userInfo";

var scheme = "wx7779c7c063a9d4d9";
...
function _showProgress(title, text, modal, animationType) {
	...
	var _modal = ( typeof arguments[2] == "undefined" || arguments[2] == null) ? false : arguments[2];
...
function _ajax(url, method, data, dataType, _call, headers, cache, timeout, tag) {
	...
	//_data.XDEBUG_SESSION= 14341;
	//_data.values.XDEBUG_SESSION = 17211;
	...
	api.ajax({
		...
		console.log("ajax请求链接：" + url);

		console.log("ajax请求数据：" + JSON.stringify(_data));

		if (ret) {
			console.log("ajax返回数据：" + JSON.stringify(ret));
			if (ret.error && ret.error == 301) {
				needLogin();
			} else if ( typeof _call == "function") {
				_call(ret);
			} else {
			...
			console.log("ajax返回错误：" + JSON.stringify(err));
			api.hideProgress();
...
function needaddress() {
	...
	api.openWin({
		name : 'address',
		url : '../../html/checkout/ckaddress_win.html'
	})
}

function _openGood(id) {
	if (id) {
		api.openWin({
			name : 'goods',
...
function _openSupplierHome(id) {

	if (id) {
		api.openWin({
			name : 'supplierhome',
			url : '../../html/supplier/supplierhome_win.html',
			pageParam : {
				id : id
			}
		});
...
function _supplierCatalog(id) {

	if (id) {
		api.openWin({
			name : 'suppliercatalog',
			url : '../../html/supplier/suppliercatalog_win.html',
			pageParam : {
				id : id
			}
		});
...
function _supplierCategory(sid) {
	if (sid) {
		api.openWin({
			name : 'suppliercategory',
			url : '../../html/supplier/suppliercategory_win.html',
			pageParam : {
				sid : sid
			}
		});
...
function _openGoodtaocan(id) {
	if (id) {
		api.openWin({
			name : 'taocan',
			url : '../../html/product/taocan_win.html',
			pageParam : {
				id : id
			}
		});
...
function _openCart() {
	_openWin('cart', '../../html/cart/cart_win.html');
}
...
function _delRec(id) {
	api.confirm({
		...
		if (index == 1) {
			_showProgress();
			_ajax(api_url + 'flow.php?step=drop_goods&id=' + id, "get", {
...
function clearcart() {
	api.confirm({
		...
		if (index == 1) {
			_showProgress();
			_ajax(api_url + 'flow.php?step=clearcart', "get", {

			}, "json", function(ret) {
				_sendEvent('clearcartdone');
				_toast("清空成功！");
				api.hideProgress();
			});
		} else {
...
function back_order(id) {
	$api.setStorage('back_order_id',id);
	if (id) {
		api.openWin({
			name : 'backorder',
			url : '../../html/user/backorder_win.html',
			pageParam : {
				id : id
			}
		});
...
function cancelOrder(id) {
	api.confirm({
		...
		if (index == 1) {
			_showProgress();
			_ajax(api_url + 'user.php?act=cancel_order&order_id=' + id, "get", {

			}, "json", function(ret) {
				//getcartbody();
				_sendEvent('cancelordered');
				_toast(ret.content);
				api.hideProgress();
...
function affirm_received(id) {
	api.confirm({
		...
		if (index == 1) {
			_showProgress();
			_ajax(api_url + 'user.php?act=affirm_received&order_id=' + id, "get", {

			}, "json", function(ret) {
				//getcartbody();
				_sendEvent('orderaffirmed');
				_toast(ret.content);
				api.hideProgress();
...
function my_comment(id) {
	...
	api.openWin({
		name : 'comment',
		url : '../../html/comment/commentlist_win.html',
	});
}

function comment_order(rec_id, goods_id) {

	api.openWin({
		name : 'commentorder',
		url : '../../html/comment/commentorder_win.html',
		pageParam : {
			rec_id : rec_id,
			goods_id : goods_id
...
function shaidan_send(rec_id, goods_id) {

	api.openWin({
		name : 'shaidan',
		url : '../../html/comment/shaidan_win.html',
		pageParam : {
			rec_id : rec_id,
			goods_id : goods_id
...
function collection(id) {
	...
	api.openWin({
		name : 'collection',
		url : '../../html/user/collection_win.html'
	});
}
