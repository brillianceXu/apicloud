...
function addToCart(goodsId, parentId, isnowbuy) {
	...
	if (one_buy == 1) {
		_ajax(api_url + 'flow.php', "post", {
			...
			console.log('one_buy:' + JSON.stringify(ret));
			addToCartResponse(ret);
...
function addToCartResponse(result) {
	...
	if (result.error > 0) {
		...
		if (result.one_step_buy == '1') {
			...
			_showProgress();
			var d = new Date();
			url_get = api_url + 'flow.php?step=checkout&sel_cartgoods=' + result.rec_id;
...
function collect(goodsId) {
	...
	_showProgress();
	_ajax(api_url + 'user.php?act=collect&id=' + goodsId, "get", {
...
function addPackageToCart(packageId) {
	...
	_showProgress();
	//alert(JSON.stringify(package_info));
	//		url_get = api_url + 'flow.php?step=add_package_to_cart';
	//
	//
	//		var data = {'package_info':package_info};
	//		_ajax(url_get, "post", {
	//			'values':data
	//		}, "json", function(ret) {
	//
...
function deleteCollection(goods_id) {
	...
	api.confirm({
		...
		if (index == 1) {
			_showProgress();
			_ajax(api_url + 'user.php?act=delete_collection&collection_id=' + goods_id, "get", {

			}, "json", function(ret) {
				//getcartbody();

				_sendEvent('delcollectiondone');
				_toast(ret.message);
				api.hideProgress();
				_setStorage('delcollectionid', goods_id);
			});
		} else {
...
function deleteFollow(shop_id) {
	...
	//	if (confirm("确定要删除吗？")) {
	//		_showProgress();
	//		_ajax(api_url + "user.php?act=del_follow&rec_id=" + shop_id, "post", {
	//				values:ldata
	//		}, "json", function(ret) {
	//			//alert(JSON.stringify(ret));
	//			api.hideProgress();
	//			deleteFollowResponse(ret);
	//		});
	//
	...
	api.confirm({
		...
		if (index == 1) {
			...
			_ajax(api_url + "user.php?act=del_follow&rec_id=" + shop_id, "post", {
				...
				//alert(JSON.stringify(ret));
				_sendEvent('delfollowdone');
				api.hideProgress();
				_toast(ret.content);

				_setStorage('delfollowid', shop_id);
			});
