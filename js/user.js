...
function submitMsg() {
	...
	if (msg.length > 0) {
		...
		_showProgress();
					var formdata = $("#formMsg").serialize();
					//_alert(formdata);

					_ajax(api_url + "user.php?act=act_add_message&" + formdata, "get", {
					}, "json", function(ret) {
						api.hideProgress();
						_alert(ret.content);
 ...
 * 会员余额申请
 */
 function submitSurplus1() {

	var formdata = $("#formSurplus").serialize();
	...
	_ajax(api_url + "user.php?act=act_account&" + formdata, "get", {

	}, "json", function(ret) {
		api.hideProgress();

		checkpay(ret.content.amount,ret.content.pay_id);
	});
 }
...
function addBonus() {
	...
	if (bonus_sn.length == 0) {
		...
		if (!reg.test(bonus_sn)) {
			...
			_showProgress();
					var formdata = $("#addBouns").serialize();


					_ajax(api_url + "user.php?act=act_add_bonus&" + formdata, "get", {
					}, "json", function(ret) {
						api.hideProgress();
...
function checkMobilePhoneExist(mobile, callback) {
	...
	$.post(api_url + 'user.php?act=check_mobile', {
		mobile_phone : mobile
	}, function(result) {
...
function sendEmailCode(emailObj, emailCodeObj, sendButton) {
	checkEmail(emailObj, function(result) {
		if (result) {
			// 发送邮件
			var url = api_url + 'user.php?act=send_email_code&XDEBUG_SESSION_START=ECLIPSE_DBGP';
			$.post(url, {
				email : emailObj.val()
			}, function(result) {
				if (result == 'ok') {
...
function sendMobileCode(mobileObj, sendButton) {
	...
	checkMobilePhone(mobileObj, function(result) {
		if (result) {
			// 发送邮件
			var url =api_url + 'user.php?act=send_mobile_code&XDEBUG_SESSION_START=ECLIPSE_DBGP';
			$.post(url, {
...
function countdown(obj, msg) {
	...
	document.getElementById('updateheaderimg').innerHTML=`<canvas id="canvas" width="150" height="150" style="display:none"></canvas><input type="file" id="headimgfile" style="display:none"><form id="changeHeadImg" method="post" style="display:none"><input type="hidden" name="headimg" id="headimgvalue"><input type="hidden" name="act" value="act_edit_img"></form>`;
	// 压缩图片需要的一些元素和对象
	...
	img.onload = function () {
	    ...
	    document.getElementById('headimgvalue').value=src;

		    alert('正在处理，请稍后！');

		  var form=document.getElementById('changeHeadImg');
		    ...
		    $.ajax({
		      ...
		      success:function(ret){
						...
						$api.setStorage('headimgurl',ret.url);

						$('#headimgurl').attr('src',ret.url);
		...
		document.getElementById('headimgfile').addEventListener('change',function(event){
